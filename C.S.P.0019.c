...
void Searching(FILE *fi, FILE *fo, int n, char lastName[][30], int status[n])
{
    int rooms = 0;
    char temp[20];
    fscanf(fi, "%s\n", temp);
...
void listCus(FILE *fi, FILE *fo, int n, char lastName[][30], int status[n], int occupants[n])
{
...
int main()
{
    ...
    do
    {
        ...
        if (strcmp(s, "SEARCH") == 0)
            Searching(fi, fo, n, lastName, status);
        if (strcmp(s, "PRINTOCCUPANCY") == 0)
            listCus(fi, fo, n, lastName, status, occupants);
        temp++;
    } while (temp < n);
